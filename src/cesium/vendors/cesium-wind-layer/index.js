...
var import_cesium6 = require("cesium");

// src/windParticlesComputing.ts
var import_cesium3 = require("cesium");
...
var ShaderManager = class {
  static getCalculateSpeedShader() {
    return new import_cesium.ShaderSource({
...
var import_cesium2 = require("cesium");
var CustomPrimitive = class {
  constructor(options) {
    ...
    this.autoClear = (0, import_cesium2.defaultValue)(options.autoClear, false);
    ...
    if (this.autoClear) {
      this.clearCommand = new import_cesium2.ClearCommand({
        color: new import_cesium2.Color(0, 0, 0, 0),
        depth: 1,
        framebuffer: this.framebuffer,
        pass: import_cesium2.Pass.OPAQUE
      });
    }
  }
  createCommand(context) {
    if (this.commandType === "Draw") {
      const vertexArray = import_cesium2.VertexArray.fromGeometry({
        context,
        geometry: this.geometry,
        attributeLocations: this.attributeLocations,
        bufferUsage: import_cesium2.BufferUsage.STATIC_DRAW
      });
      const shaderProgram = import_cesium2.ShaderProgram.fromCache({
        context,
        vertexShaderSource: this.vertexShaderSource,
        fragmentShaderSource: this.fragmentShaderSource,
        attributeLocations: this.attributeLocations
      });
      const renderState = import_cesium2.RenderState.fromCache(this.rawRenderState);
      return new import_cesium2.DrawCommand({
        owner: this,
        vertexArray,
        primitiveType: this.primitiveType,
        modelMatrix: import_cesium2.Matrix4.IDENTITY,
        renderState,
        shaderProgram,
        framebuffer: this.framebuffer,
      ...
      return new import_cesium2.ComputeCommand({
        owner: this,
        fragmentShaderSource: this.fragmentShaderSource,
        uniformMap: this.uniformMap,
  ...
  setGeometry(context, geometry) {
    ...
    if ((0, import_cesium2.defined)(this.commandToExecute)) {
      this.commandToExecute.vertexArray = import_cesium2.VertexArray.fromGeometry({
        context,
        geometry: this.geometry,
        attributeLocations: this.attributeLocations,
        bufferUsage: import_cesium2.BufferUsage.STATIC_DRAW
  ...
  update(frameState) {
    ...
    if (!this.show || !(0, import_cesium2.defined)(frameState)) {
      return;
    }
    if (!(0, import_cesium2.defined)(this.commandToExecute)) {
      this.commandToExecute = this.createCommand(frameState.context);
    }
    if ((0, import_cesium2.defined)(this.preExecute)) {
      this.preExecute();
    }
    ...
    if ((0, import_cesium2.defined)(this.clearCommand)) {
  ...
  destroy() {
    ...
    return (0, import_cesium2.destroyObject)(this);
  }
};
...
var WindParticlesComputing = class {
  constructor(context, windData, options, viewerParameters, scene) {
    ...
    this.frameRateMonitor = new import_cesium3.FrameRateMonitor({
      scene,
      samplingWindow: 1,
      quietPeriod: 0
    });
  ...
  createWindTextures() {
    const options = {
      ...
      pixelFormat: import_cesium3.PixelFormat.RED,
      pixelDatatype: import_cesium3.PixelDatatype.FLOAT,
      flipY: this.options.flipY ?? false,
      sampler: new import_cesium3.Sampler({
        minificationFilter: import_cesium3.TextureMinificationFilter.LINEAR,
        magnificationFilter: import_cesium3.TextureMagnificationFilter.LINEAR
      })
    };
    this.windTextures = {
      U: new import_cesium3.Texture({
        ...options,
        source: {
          arrayBufferView: new Float32Array(this.windData.u.array)
        }
      }),
      V: new import_cesium3.Texture({
        ...options,
        source: {
  ...
  createParticlesTextures() {
    const options = {
      ...
      pixelFormat: import_cesium3.PixelFormat.RGBA,
      pixelDatatype: import_cesium3.PixelDatatype.FLOAT,
      flipY: false,
      source: {
      ...
      sampler: new import_cesium3.Sampler({
        minificationFilter: import_cesium3.TextureMinificationFilter.NEAREST,
        magnificationFilter: import_cesium3.TextureMagnificationFilter.NEAREST
      })
    };
    this.particlesTextures = {
      previousParticlesPosition: new import_cesium3.Texture(options),
      currentParticlesPosition: new import_cesium3.Texture(options),
      nextParticlesPosition: new import_cesium3.Texture(options),
      postProcessingPosition: new import_cesium3.Texture(options),
      particlesSpeed: new import_cesium3.Texture(options)
    };
  }
    ...
    this.primitives = {
      calculateSpeed: new CustomPrimitive({
        ...
        uniformMap: {
          ...
          uRange: () => new import_cesium3.Cartesian2(this.windData.u.min, this.windData.u.max),
          vRange: () => new import_cesium3.Cartesian2(this.windData.v.min, this.windData.v.max),
          speedRange: () => new import_cesium3.Cartesian2(this.windData.speed.min, this.windData.speed.max),
          ...
          dimension: () => new import_cesium3.Cartesian2(this.windData.width, this.windData.height),
          minimum: () => new import_cesium3.Cartesian2(this.windData.bounds.west, this.windData.bounds.south),
          maximum: () => new import_cesium3.Cartesian2(this.windData.bounds.east, this.windData.bounds.north)
        },
      ...
      postProcessingPosition: new CustomPrimitive({
        ...
        uniformMap: {
          ...
          dataLonRange: () => new import_cesium3.Cartesian2(this.windData.bounds.west, this.windData.bounds.east),
          dataLatRange: () => new import_cesium3.Cartesian2(this.windData.bounds.south, this.windData.bounds.north),
...
var import_cesium4 = require("cesium");
var WindParticlesRendering = class {
  ...
  createRenderingTextures() {
    const colorTextureOptions = {
      ...
      pixelFormat: import_cesium4.PixelFormat.RGBA,
      pixelDatatype: import_cesium4.PixelDatatype.UNSIGNED_BYTE
    };
    const depthTextureOptions = {
      context: this.context,
      width: this.context.drawingBufferWidth,
      height: this.context.drawingBufferHeight,
      pixelFormat: import_cesium4.PixelFormat.DEPTH_COMPONENT,
      pixelDatatype: import_cesium4.PixelDatatype.UNSIGNED_INT
    };
    return {
      segmentsColor: new import_cesium4.Texture(colorTextureOptions),
      segmentsDepth: new import_cesium4.Texture(depthTextureOptions)
    };
  }
  createRenderingFramebuffers() {
    return {
      segments: new import_cesium4.Framebuffer({
        context: this.context,
  ...
  createColorTableTexture() {
    const colorTableData = new Float32Array(this.options.colors.flatMap((color) => {
      const cesiumColor = import_cesium4.Color.fromCssColorString(color);
      return [cesiumColor.red, cesiumColor.green, cesiumColor.blue, cesiumColor.alpha];
    }));
    return new import_cesium4.Texture({
      context: this.context,
      width: this.options.colors.length,
      height: 1,
      pixelFormat: import_cesium4.PixelFormat.RGBA,
      pixelDatatype: import_cesium4.PixelDatatype.FLOAT,
      sampler: new import_cesium4.Sampler({
        minificationFilter: import_cesium4.TextureMinificationFilter.LINEAR,
        magnificationFilter: import_cesium4.TextureMagnificationFilter.LINEAR,
        wrapS: import_cesium4.TextureWrap.CLAMP_TO_EDGE,
        wrapT: import_cesium4.TextureWrap.CLAMP_TO_EDGE
      }),
      source: {
        width: this.options.colors.length,
        height: 1,
  ...
  createSegmentsGeometry() {
    ...
    const geometry = new import_cesium4.Geometry({
      attributes: new import_cesium4.GeometryAttributes({
        st: new import_cesium4.GeometryAttribute({
          componentDatatype: import_cesium4.ComponentDatatype.FLOAT,
          componentsPerAttribute: 2,
          values: st
        }),
        normal: new import_cesium4.GeometryAttribute({
          componentDatatype: import_cesium4.ComponentDatatype.FLOAT,
  ...
  createRawRenderState(options) {
    return import_cesium4.Appearance.getDefaultRenderState(true, false, {
      viewport: void 0,
      depthTest: void 0,
      depthMask: void 0,
      blending: void 0,
  ...
  createPrimitives() {
    const segments = new CustomPrimitive({
      ...
      primitiveType: import_cesium4.PrimitiveType.TRIANGLES,
      uniformMap: {
        ...
        domain: () => {
          const domain = new import_cesium4.Cartesian2(this.options.domain?.min ?? this.computing.windData.speed.min, this.options.domain?.max ?? this.computing.windData.speed.max);
          return domain;
        },
        displayRange: () => {
          const displayRange = new import_cesium4.Cartesian2(
        ...
        lineWidth: () => {
          ...
          return new import_cesium4.Cartesian2(width.min, width.max);
        },
        lineLength: () => {
          const length = this.options.lineLength || DefaultOptions.lineLength;
          return new import_cesium4.Cartesian2(length.min, length.max);
        },
        is3D: () => this.viewerParameters.sceneMode === import_cesium4.SceneMode.SCENE3D,
  ...
  onParticlesTextureSizeChange() {
    ...
    const vertexArray = import_cesium4.VertexArray.fromGeometry({
      context: this.context,
      geometry,
      attributeLocations: this.primitives.segments.attributeLocations,
      bufferUsage: import_cesium4.BufferUsage.STATIC_DRAW
    });
...
var import_cesium5 = require("cesium");
var WindParticleSystem = class {
  ...
  clearFramebuffers() {
    const clearCommand = new import_cesium5.ClearCommand({
      color: new import_cesium5.Color(0, 0, 0, 0),
      depth: 1,
      framebuffer: void 0,
      pass: import_cesium5.Pass.OPAQUE
    });
...
var _WindLayer = class _WindLayer {
  ...
  constructor(viewer, windData, options) {
    ...
    this.viewerParameters = {
      lonRange: new import_cesium6.Cartesian2(-180, 180),
      latRange: new import_cesium6.Cartesian2(-90, 90),
      pixelSize: 1e3,
  ...
  updateViewerParameters() {
    ...
    for (const corner of corners) {
      const cartesian = scene.camera.pickEllipsoid(
        new import_cesium6.Cartesian2(corner.x, corner.y),
      ...
      const lon = import_cesium6.Math.toDegrees(cartographic.longitude);
      const lat = import_cesium6.Math.toDegrees(cartographic.latitude);
    ...
    if (!isOutsideGlobe) {
      const lonRange = new import_cesium6.Cartesian2(
        Math.max(this.windData.bounds.west, minLon),
        Math.min(this.windData.bounds.east, maxLon)
      );
      const latRange = new import_cesium6.Cartesian2(
  ...
  zoomTo(duration = 0) {
    if (this.windData.bounds) {
      const rectangle = import_cesium6.Rectangle.fromDegrees(
